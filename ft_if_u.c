...
/*   Updated: 2021/01/19 10:17:10 by max              ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	ft_if_u(int unsigned integer, t_flags flags)
{
	char	*str;
	int		len;
	int		count;
	...
	count = 0;
	if (!(str = ft_itoa_base(integer, 10)))
		return (-1);
	len = ft_strlen(str);
	if (integer == 0 && flags.dot >= 0)
		len = 0;
	if (flags.dot >= 0 && flags.minus == 1)
		count += ft_if_u_min_dot(flags, str, len);
	else if (flags.dot < 0 && flags.minus == 1)
		count += ft_if_u_min_ndot(flags, str, len);
	if (flags.dot >= 0 && flags.minus == 0)
		count += ft_if_u_nmin_dot(flags, str, len);
	else if (flags.dot < 0 && flags.minus == 0)
		count += ft_if_u_nmin_ndot(flags, str, len);
	free(str);
	return (count);
}
